...
export class DateTime {
   ...
   * @param formatString (可选)格式化模板 默认为`Config.dateTimeFormatter`
   */
  static formatFromSecond(timeStamp: number, formatString?: DateTimeFormatter | string): string {
  ...
  static formatFromMilliSecond(
    timeStamp: number,
    formatString?: DateTimeFormatter | string,
  ): string {
   ...
   * @param formatString (可选)格式化模板 默认为`Config.dateTimeFormatter`
   */
  static formatFromDate(date: Date | string, formatString?: DateTimeFormatter | string): string {
    if (!formatString) {
      formatString = Config.dateTimeFormatter
    }
    switch (typeof date) {
      case 'string':
        date = new Date(date)
