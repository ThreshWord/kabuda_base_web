...
export class Alert {
   ...
   * # 可选的确认图标类型
   */
  protected icon = feedBackType.NONE
  ...
  success(content?: string, title?: string): Promise<void> {
    this.icon = feedBackType.SUCCESS
  ...
  warning(content?: string, title?: string): Promise<void> {
    this.icon = feedBackType.WARNING
    return this.alert(content, title)
  }
  ...
  protected getConfig(): ElMessageBoxOptions {
    ...
    return {
      ...
      type: this.confirmButtonClass ? feedBackType.NONE : this.icon,
      draggable: true,
